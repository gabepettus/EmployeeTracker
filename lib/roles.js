...
async function updateRole() {
  ...
  switch (update) {
    case "Title":
      const title = await inquirer.prompt(questions.updateTitle);
      value.push(title);
      break;
    case "Salary":
      const salary = await inquirer.prompt(questions.updateSalary);
      value.push(salary);
      break;
    case "Department":
      department.getDeptList();
      const department = await inquirer.prompt(questions.updateDepartment);
      break;
    default:
      break;
  }
  ...
  questions.updateRole[1].choices = choices;
  const ujjj = await inquirer.prompt(questions.addRole);
  console.log("add",add);

  let values = [add.roleName,add.salary,add.deptId];
  try {
    const sql = `UPDATE role (title, salary, dept_id) VALUES (?,?,?) WHERE id = ?`;
    await db.query(sql,values);
    console.log(`\nSuccessfully created a new role\n`);
  } catch (err) {
    console.log("error with role insert",err);
