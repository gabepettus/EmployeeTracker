...
async function addEmployee() {
  ...
  //get userinfo for dept
  const deptResults = await dept.getDeptList(formatForInq);

  let deptChoices = [];
  deptResults.forEach(cur => {
    deptChoices.push(cur);
  });

  questions.selectDept[0].choices = deptChoices;

  const neDept = await inquirer.prompt(questions.selectDept);
  values.push(neDept.dept);

  // get employee list
  const emplResults = await getEmplListByDept(neDept.dept);
...
async function removeEmployee() {
  ...
  if (confirm) {
    const sql = `DELETE FROM employee WHERE name = "${dept.emplName}"`;

    try {
      await db.query(sql);
      console.log(`\nSuccessully removed departemnt ${dept.emplName}\n`);
    } catch (err) {
...
async function getEmplListByDept(dept) {
  ...
  console.log(dept);
  const sql = "SELECT id, CONCAT(last_name,\",\",first_name) AS empl FROM employee WHERE role_id = ( SELECT id from role where dept_id = ?) ORDER BY last_name";
  try {
...
async function getEmplList(inq) {
  let empl;
  const sql = "SELECT id, first_name, last_name, CONCAT(last_name,\",\",first_name) AS empl FROM employee ORDER BY last_name";
